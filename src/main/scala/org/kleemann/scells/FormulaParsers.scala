...
object FormulaParsers extends RegexParsers {
  ...
  def cell: Parser[Coord] =
    // TODO: support columns of multiple letters
    """[A-Za-z]\d+""".r ^^ { s =>
  ...
  def expr: Parser[Formula] =
    // TODO: small error in that range should not be an accepted top level value
    // but makes parsing easier
    range | cell | number | application
    
  def textual: Parser[Textual] =
    // I suppose Textual is passed a string argument?
    """[^=].*""".r ^^ Textual
    
  def formula: Parser[Formula] =
    // TODO: not sure what > means below
    number | textual | "="~>expr
